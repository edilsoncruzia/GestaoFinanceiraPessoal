    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[[3, "3 meses"], [6, "6 meses"], [12, "12 meses"]].map(([v, l]) => (
          <button key={v} onClick={() => setHorizon(v)} style={{ flex: 1, padding: "8px 0", borderRadius: 20, fontSize: 14, fontWeight: 500, border: "1px solid " + (horizon === v ? COLORS.green : COLORS.line), background: horizon === v ? COLORS.green : "transparent", color: horizon === v ? "#fff" : COLORS.muted }}>{l}</button>
        ))}
      </div>

      {finalRow && (
        <Card style={{ marginBottom: 16 }}>
          <p style={{ fontSize: 14, color: COLORS.muted, margin: "0 0 4px" }}>Saldo projetado em {monthLabelFull(finalRow.month)}</p>
          ...
          {firstNegative
            ? <p style={{ fontSize: 13, color: COLORS.rust, margin: 0 }}>Atenção: no ritmo atual de compromissos previstos, o saldo fica negativo em {monthLabelFull(firstNegative.month)}.</p>
            : <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Considerando salários, aluguel, contas e parcelas já cadastrados no Previsto.</p>}
        </Card>
      )}

      <Card style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 14, fontWeight: 500, margin: "0 0 10px" }}>Receitas x despesas projetadas</p>
        ...
        <p style={{ fontSize: 12, color: COLORS.muted, margin: "8px 0 0" }}>A linha escura é o saldo acumulado da família mês a mês.</p>
      </Card>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {rows.map((r) => (
          <Card key={r.month} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px" }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 15, fontWeight: 500, margin: 0, textTransform: "capitalize" }}>{monthLabelFull(r.month)}</p>
              <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}><span style={{ color: COLORS.income }}>+{fmt(r.receitas)}</span> · <span style={{ color: COLORS.rust }}>−{fmt(r.despesas)}</span></p>
            </div>
            <p style={{ fontSize: 15, fontWeight: 600, margin: 0, color: r.saldoAcumulado >= 0 ? COLORS.income : COLORS.rust }}>{fmt(r.saldoAcumulado)}</p>
          </Card>
        ))}
      </div>
    </div>
