    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {[["todos", "Todos"], ["conta", "Contas"], ["cartao", "Cartões"], ["reservas", "Reservas"]].map(([v, l]) => (
          <button key={v} onClick={() => setFilter(v)} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 14, fontWeight: 500, border: "1px solid " + (filter === v ? COLORS.green : COLORS.line), background: filter === v ? COLORS.green : "transparent", color: filter === v ? "#fff" : COLORS.ink }}>{l}</button>
        ))}
      </div>
      ...
      {totalReserved > 0 && (
        <Card style={{ marginBottom: 16, background: COLORS.amber + "12", borderColor: COLORS.amber, padding: "12px 14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: COLORS.amber, flex: 1 }}>Total guardado (Reservas)</span>
            <span className="serif" style={{ fontSize: 19, fontWeight: 600, color: COLORS.ink }}>{fmt(totalReserved)}</span>
          </div>
          <p style={{ fontSize: 12, color: COLORS.muted, margin: "4px 0 0" }}>Não entra no saldo disponível para gastar. Gerencie em "Reservas".</p>
        </Card>
      )}
            ...
            <Card key={a.id}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 15, fontWeight: 500, margin: 0 }}>{a.name}</p>
                  <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 2px" }}>
              ...
              {a.type === "conta" && (
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "1px solid " + COLORS.line }}>
                  <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>{a.countInAvailable === false ? "Saldo (fora do disponível)" : "Saldo"}</p>
                  <p style={{ fontSize: 14, margin: 0, color: saldo >= 0 ? COLORS.income : COLORS.rust }}>{fmt(saldo)}</p>
                </div>
              )}
              ...
              {a.type === "cartao" && (
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10 }}>
                    <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Fatura atual: {fmt(a.currentInvoice || 0)}</p>
                    <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Limite: {fmt(a.limit || 0)}</p>
                  </div>
                  <ProgressBar pct={a.limit > 0 ? Math.min(100, (used / a.limit) * 100) : 0} color={st ? st.color : COLORS.muted} />
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                    <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Uso: {fmt(used)} · Falta para o limite: <strong style={{ color: COLORS.ink }}>{fmt(Math.max(0, (a.limit || 0) - used))}</strong></p>
              ...
              {onViewStatements && (
                <button onClick={() => onViewStatements(a)} style={{ width: "100%", marginTop: 10, padding: "8px 0", borderRadius: 10, border: "1px solid " + COLORS.line, background: "transparent", color: COLORS.green, fontSize: 13.5, fontWeight: 500 }}>Ver extrato</button>
              )}
            </Card>
          );
        })}
      ...
      <button onClick={onAdd} style={{ width: "100%", padding: "12px 0", borderRadius: 10, border: "1px dashed " + COLORS.line, background: "transparent", color: COLORS.green, fontSize: 15, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
