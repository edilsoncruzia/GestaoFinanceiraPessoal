...
// no mês seguinte.
export function ReservaView({ onBack, month, onMonthChange, reserva, config, salario, onSaveConfig, saldoDoMes }) {
  ...
  return (
    <div>
      ...
      <MonthNav month={month} onChange={onMonthChange} saldoDoMes={saldoDoMes} />
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
        ...
        <div>
          <p className="serif" style={{ fontSize: 20, fontWeight: 600, margin: 0, color: COLORS.ink }}>Reserva mínima</p>
          <p style={{ fontSize: 13, color: COLORS.muted, margin: "2px 0 0" }}>Funciona como o limite do cartão: você lança nela o que usar e o disponível cai. A reserva é mensal — não acumula de um mês para o outro.</p>
        </div>
      </div>
      ...
      <Card style={{ marginBottom: 12, padding: "14px 16px", borderLeft: "4px solid " + corUso }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          ...
          <p style={{ fontSize: 14.5, fontWeight: 700, margin: 0, color: COLORS.ink, flex: 1 }}>Disponível em {monthLabelFull(month)}</p>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
          <span style={{ fontSize: 13, color: COLORS.muted }}>Usado {fmt(reserva.usado)}</span>
          <span style={{ fontSize: 13, color: COLORS.muted }}>Aporte do mês {fmt(reserva.aporte)}</span>
        </div>
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid " + COLORS.line }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
            ...
            <span style={{ fontSize: 13, color: COLORS.ink }}>+ {fmt(reserva.aporte)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
            <span style={{ fontSize: 13, color: COLORS.muted }}>Usado na reserva (Despesa)</span>
            <span style={{ fontSize: 13, color: COLORS.ink }}>− {fmt(reserva.usado)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, color: COLORS.muted }}>Saldo do mês</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: reserva.saldo < 0 ? COLORS.rust : COLORS.income }}>{fmt(reserva.saldo)}</span>
          </div>
          {(reserva.receitaDoMesAnterior > 0 || reserva.despesaDoMesAnterior > 0) && (
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
              <span style={{ fontSize: 13, color: COLORS.muted }}>Fechamento do mês anterior</span>
              <span style={{ fontSize: 13, color: reserva.despesaDoMesAnterior > 0 ? COLORS.rust : COLORS.green }}>
      ...
      <Card style={{ marginBottom: 12, padding: "14px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          ...
          <p style={{ fontSize: 14.5, fontWeight: 700, margin: 0, color: COLORS.ink, flex: 1 }}>Aporte mensal da reserva (valor fixo)</p>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
          <button type="button" onClick={() => setLimite(String(sugerido))} disabled={sugerido <= 0}
            style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13.5, padding: "9px 0", borderRadius: 10, border: "1px solid " + COLORS.line, background: "transparent", color: sugerido > 0 ? COLORS.green : COLORS.muted, fontWeight: 500 }}>
            <RefreshCw size={13} />Usar 15% do salário{sugerido > 0 ? " (" + fmt(sugerido) + ")" : ""}
          </button>
          <button type="button" onClick={() => setLimite("")} style={{ flex: 1, fontSize: 13.5, padding: "9px 0", borderRadius: 10, border: "1px solid " + COLORS.line, background: "transparent", color: COLORS.muted, fontWeight: 500 }}>
            Limpar
          </button>
        </div>
        ...
        <p style={{ fontSize: 12.5, color: COLORS.muted, margin: "8px 0 0" }}>
          {reserva.configurado
      ...
      <Card style={{ marginBottom: 12, padding: "14px 16px", background: "#3B6E8F0F", border: "1px solid #3B6E8F22" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
          ...
          <p style={{ fontSize: 14.5, fontWeight: 700, margin: 0, color: COLORS.ink, flex: 1 }}>Fechamento do mês</p>
        </div>
        {reserva.saldo > 0 ? (
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
            <ShieldCheck size={16} color={COLORS.green} style={{ marginTop: 1, flexShrink: 0 }} />
            <span style={{ fontSize: 13.5, color: COLORS.ink, lineHeight: 1.5 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
            ...
            <span style={{ fontSize: 13.5, color: COLORS.rust, lineHeight: 1.5 }}>
      ...
      <p style={{ fontSize: 13, fontWeight: 600, color: COLORS.muted, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: 0.4 }}>O que consumiu a reserva</p>
      {reserva.itens.length === 0 ? (
        <Card style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <ShieldCheck size={18} color={COLORS.green} />
          <p style={{ fontSize: 14, margin: 0 }}>Nada lançado na reserva em {monthLabelFull(month)}.</p>
        </Card>
      ) : (
          ...
          {reserva.itens.map((i) => (
            <Card key={i.occId} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 14.5, fontWeight: 500, margin: 0, color: COLORS.ink }}>{i.description}</p>
                <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>{fmtDate(i.dueDate)} · dia {i.dia} · {memberLabel(i.memberId)}</p>
              </div>
              <span style={{ fontSize: 14.5, fontWeight: 600, color: COLORS.ink, whiteSpace: "nowrap" }}>{fmt(i.amount)}</span>
            </Card>
          ))}
          <p style={{ fontSize: 12, color: COLORS.muted, margin: "2px 0 0" }}>Para lançar mais, use "Novo previsto" com a forma de pagamento "Reserva mínima".</p>
        </div>
      )}
    </div>
  );
