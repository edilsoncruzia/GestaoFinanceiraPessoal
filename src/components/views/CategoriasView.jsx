...
const typeBtn = (active) => ({ flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 14, fontWeight: 500, border: "1px solid " + (active ? COLORS.green : COLORS.line), background: active ? COLORS.green : "transparent", color: active ? "#fff" : COLORS.ink });
...
export function CategoriasView({ onBack, onSave, onDelete }) {
  ...
  function renderGroup(title, list) {
    ...
    return (
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: 13, fontWeight: 600, color: COLORS.muted, margin: "0 0 8px", textTransform: "uppercase", letterSpacing: 0.4 }}>{title} ({list.length})</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {list.map(([key, c]) => (
            <Card key={key}>
              {confirming === key ? (
                <div>
                  <p style={{ fontSize: 14, margin: "0 0 10px" }}>Excluir "{c.label}"? Lançamentos já feitos passam a mostrar o nome da categoria como está salvo.</p>
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => { onDelete(key); setConfirming(null); }} style={{ flex: 1, fontSize: 13, padding: "8px 0", borderRadius: 8, border: "none", background: COLORS.rust, color: "#fff", fontWeight: 500 }}>Excluir</button>
                  </div>
                </div>
              ) : (
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 15, fontWeight: 500, margin: 0 }}>{c.label}</p>
  ...
  return (
    <div>
      ...
      {rows.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, textAlign: "center", padding: "12px 0" }}>Nenhuma categoria cadastrada ainda.</p>}

      {!showForm && (
        <button onClick={openNew} style={{ width: "100%", padding: "12px 0", borderRadius: 10, border: "1px dashed " + COLORS.line, background: "transparent", color: COLORS.green, fontSize: 15, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      {showForm && (
        <Card>
          <p style={{ fontSize: 15, fontWeight: 500, margin: "0 0 12px" }}>{editingKey ? "Editar categoria" : "Nova categoria"}</p>
