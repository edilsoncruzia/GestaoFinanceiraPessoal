...
const inputStyle = { width: "100%", padding: "10px 12px", borderRadius: 10, border: "1px solid " + COLORS.line, background: COLORS.card, fontSize: 15, outline: "none", fontFamily: "inherit" };
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <Card style={{ flex: 1, padding: "10px 12px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>Notas</p>
        </Card>
        <Card style={{ flex: 1, padding: "10px 12px", textAlign: "center" }}>
          <p className="serif" style={{ fontSize: 22, fontWeight: 600, margin: 0, color: COLORS.green }}>{doneCount}</p>
          <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>Concluídas</p>
        </Card>
        <Card style={{ flex: 1, padding: "10px 12px", textAlign: "center" }}>
          <p className="serif" style={{ fontSize: 22, fontWeight: 600, margin: 0, color: COLORS.amber }}>{total - doneCount}</p>
          <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>Pendentes</p>
        </Card>
      </div>

      <Card style={{ marginBottom: 14 }}>
        <p style={{ fontSize: 14, fontWeight: 500, margin: "0 0 8px" }}>Nova ideia</p>
        ...
        <p style={{ fontSize: 12, color: COLORS.muted, margin: "6px 0 0" }}>Dica: cole (Ctrl+V) uma imagem para ilustrar a ideia.</p>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
          <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, padding: "8px 12px", borderRadius: 10, border: "1px solid " + COLORS.line, background: COLORS.card, color: COLORS.green, cursor: "pointer", fontWeight: 500 }}>
            <Paperclip size={14} />
          ...
          {attachment && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12, color: COLORS.muted, background: COLORS.line + "66", borderRadius: 8, padding: "4px 8px", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              <ImageIcon size={12} />
        ...
        <button onClick={submit} disabled={!text.trim()} style={{ width: "100%", marginTop: 10, padding: "10px 0", borderRadius: 10, border: "none", background: text.trim() ? COLORS.green : COLORS.line, color: text.trim() ? "#fff" : COLORS.muted, fontSize: 15, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
          <Plus size={15} /> Adicionar ideia
        </button>
      </Card>

      {sorted.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, textAlign: "center", padding: "24px 0" }}>Nenhuma ideia ainda. Vá usando o app e registre o que quiser melhorar.</p>}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {sorted.map((idea) => (
          <Card key={idea.id}>
            {confirming === idea.id ? (
              <div>
                <p style={{ fontSize: 14, margin: "0 0 10px" }}>Remover esta ideia?</p>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setConfirming(null)} style={{ flex: 1, fontSize: 13, padding: "8px 0", borderRadius: 8, border: "1px solid " + COLORS.line, background: "transparent", color: COLORS.muted }}>Cancelar</button>
                  <button onClick={() => { onDelete(idea.id); setConfirming(null); }} style={{ flex: 1, fontSize: 13, padding: "8px 0", borderRadius: 8, border: "none", background: COLORS.rust, color: "#fff", fontWeight: 500 }}>Remover</button>
                </div>
              </div>
            ) : editingId === idea.id ? (
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={() => setEditingId(null)} style={{ flex: 1, fontSize: 13, padding: "8px 0", borderRadius: 8, border: "1px solid " + COLORS.line, background: "transparent", color: COLORS.muted }}>Cancelar</button>
                  <button onClick={saveEdit} style={{ flex: 1, fontSize: 13, padding: "8px 0", borderRadius: 8, border: "none", background: COLORS.green, color: "#fff", fontWeight: 500 }}>Salvar</button>
                </div>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <button onClick={() => onToggle(idea.id)} aria-label="Marcar como feito" style={{ background: "none", border: "none", padding: 2, marginTop: 1, cursor: "pointer", flexShrink: 0 }}>
                  <span style={{ display: "block", width: 20, height: 20, borderRadius: 6, border: "2px solid " + (idea.done ? COLORS.green : COLORS.line), background: idea.done ? COLORS.green : "transparent", color: "#fff", fontSize: 13, lineHeight: "18px", textAlign: "center" }}>{idea.done ? "✓" : ""}</span>
                </button>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 15, margin: 0, color: idea.done ? COLORS.muted : COLORS.ink, textDecoration: idea.done ? "line-through" : "none", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{idea.text}</p>
                  <p style={{ fontSize: 12, color: COLORS.muted, margin: "4px 0 0" }}>#{idea.id} · {idea.date ? fmtDate(idea.date) : "sem data"}
                  ...
                  {idea.attachment && (
                    <button onClick={() => isImage(idea.attachment) ? setPreview(idea.attachment) : window.open(idea.attachment, "_blank")} style={{ background: "none", border: "none", padding: 0, marginTop: 6, display: "inline-block", cursor: isImage(idea.attachment) ? "zoom-in" : "pointer" }}>
                      {isImage(idea.attachment)
                        ...
                        : <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 13, color: COLORS.green, background: COLORS.green + "12", borderRadius: 8, padding: "5px 10px" }}><Paperclip size={12} />{idea.attachment}</span>}
                    </button>
                  )}
                </div>
