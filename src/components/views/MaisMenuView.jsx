...
export function BackRow({ onBack }) {
  return (
    <button onClick={onBack} style={{ background: "none", border: "none", display: "flex", alignItems: "center", gap: 6, color: COLORS.muted, fontSize: 14, padding: 0, marginBottom: 14 }}>
      ...
      {onLogout && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: COLORS.card, border: "1px solid " + COLORS.line, borderRadius: 14, padding: "12px 16px", marginBottom: 14 }}>
          <div style={{ minWidth: 0, marginRight: 12 }}>
            <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>Logado como</p>
            <p style={{ fontSize: 15, fontWeight: 600, margin: 0, color: COLORS.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{userName || "usuário"}</p>
          </div>
          <button onClick={onLogout} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 14, fontWeight: 600, padding: "8px 14px", borderRadius: 10, border: "1px solid " + COLORS.rust, background: "transparent", color: COLORS.rust, flexShrink: 0 }}>
            <LogOut size={15} /> Sair
          </button>
        </div>
      )}
