  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
        ...
        <p className="serif" style={{ fontSize: 21, fontWeight: 600, margin: 0, color: COLORS.ink }}>{account.name}</p>
      </div>
      ...
      {rows.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, textAlign: "center", padding: "24px 0" }}>Nenhum lançamento nesta conta.</p>}
            ...
            <Card key={t.id} style={{ padding: "10px 14px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 13, color: COLORS.muted, margin: "0 0 2px" }}>{categories[t.category]?.label || t.category} · {fmtDate(t.date)}</p>
                  {t.deductedInPayroll && <p style={{ fontSize: 12, color: COLORS.green, margin: "0 0 2px" }}>Descontado em folha</p>}
                  {fonte && <p style={{ fontSize: 13, color: COLORS.green, margin: "0 0 2px" }}>Fonte: {fonte.name}</p>}
                  {t.attachment && !isImage && <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>📎 {t.attachment}</p>}
                </div>
                <p style={{ fontSize: 15, fontWeight: 600, margin: 0, color: t.type === "income" ? COLORS.income : (t.type === "transferencia" ? "#3B6E8F" : COLORS.rust), whiteSpace: "nowrap" }}>{t.type === "income" ? "+" : t.type === "expense" ? "−" : ""}{fmt(t.amount)}</p>
              </div>
              {isImage && (
