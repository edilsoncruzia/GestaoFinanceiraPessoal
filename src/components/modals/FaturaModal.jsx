    ...
    <ModalSheet title="Pagar fatura do cartão" onClose={onClose}>
      <Card style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <p style={{ fontSize: 15, fontWeight: 600, margin: 0, color: COLORS.ink, flex: 1 }}>{item.description}</p>
          <p className="serif" style={{ fontSize: 21, fontWeight: 600, margin: 0, color: COLORS.ink }}>{fmt(total)}</p>
        </div>
        <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Vencimento {item.dueDate} · {selecionados.length} de {itens.length} lançamento(s)</p>
      </Card>
      ...
      <p style={{ fontSize: 13, fontWeight: 600, color: COLORS.muted, margin: "12px 0 8px", textTransform: "uppercase", letterSpacing: 0.4 }}>Lançamentos da fatura</p>
      {itens.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, margin: "0 0 10px" }}>Nenhum lançamento nesta fatura.</p>}
      {itens.map((c, idx) => (
        <div key={idx} style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 10, border: "1px solid " + COLORS.line, background: COLORS.card, opacity: c.incluir ? 1 : 0.55 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <p style={{ flex: 1, fontSize: 14.5, fontWeight: 500, margin: 0, color: COLORS.ink }}>{c.description}</p>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 11.5, color: COLORS.muted, margin: "0 0 2px" }}>Vencimento</p>
              <input type="date" value={c.dueDate || ""} onChange={(e) => update(idx, "dueDate", e.target.value)} style={{ ...inputStyle, padding: "7px 9px", fontSize: 13.5 }} />
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 11.5, color: COLORS.muted, margin: "0 0 2px" }}>Valor</p>
              <input type="number" min="0" step="0.01" value={c.amount} onChange={(e) => update(idx, "amount", e.target.value)} style={{ ...inputStyle, padding: "7px 9px", fontSize: 13.5, textAlign: "right" }} />
            </div>
          </div>
        </div>
      ))}
      ...
      <p style={{ fontSize: 12, color: COLORS.muted, margin: "8px 0 0" }}>Ajuste a data/valor aqui se precisar. Para mudar categoria, parcelas ou recorrência, toque no lápis do lançamento.</p>
    </ModalSheet>
  );
}
