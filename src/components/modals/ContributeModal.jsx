  ...
  return (
    <ModalSheet title="Contribuir para a meta" onClose={onClose}>
      <Card style={{ marginBottom: 14 }}>
        <p style={{ fontSize: 15, fontWeight: 500, margin: "0 0 4px" }}>{goal.name}</p>
        <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>{fmt(goal.saved)} de {fmt(goal.target)} · faltam {fmt(remaining)}</p>
      </Card>
      ...
      <p style={{ fontSize: 12, color: COLORS.muted, margin: "10px 0 0", textAlign: "center" }}>Isso também lança uma despesa de Poupança/Meta na conta escolhida.</p>
    </ModalSheet>
  );
}
