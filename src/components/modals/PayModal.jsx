  ...
  return (
    <ModalSheet title={item.type === "income" ? "Registrar recebimento" : "Registrar pagamento"} onClose={onClose}>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button onClick={() => setTab("pagamento")} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 14, fontWeight: 500, border: "1px solid " + (tab === "pagamento" ? COLORS.green : COLORS.line), background: tab === "pagamento" ? COLORS.green : "transparent", color: tab === "pagamento" ? "#fff" : COLORS.muted }}>Pagamento</button>
        <button onClick={() => setTab("extrato")} style={{ flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 14, fontWeight: 500, border: "1px solid " + (tab === "extrato" ? COLORS.green : COLORS.line), background: tab === "extrato" ? COLORS.green : "transparent", color: tab === "extrato" ? "#fff" : COLORS.muted }}>Extrato ({payments.length})</button>
      </div>

      <Card style={{ marginBottom: 14 }}>
        <p style={{ fontSize: 15, fontWeight: 500, margin: "0 0 4px" }}>{item.description}</p>
        <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Previsto {fmt(item.amount)} · já {item.type === "income" ? "recebido" : "pago"} {fmt(item.paid)} · restam {fmt(remaining)}</p>
      </Card>

      {tab === "extrato" ? (
        <div style={{ marginBottom: 14 }}>
          {payments.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, textAlign: "center", padding: "16px 0" }}>Nenhum pagamento registrado ainda.</p>}
          {payments.map((t) => {
              ...
              <Card key={t.id} style={{ padding: "10px 14px", marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 14.5, fontWeight: 500, margin: 0 }}>{fmtDate(t.date)}</p>
                    {fonte && <p style={{ fontSize: 13, color: COLORS.green, margin: "2px 0 0" }}>Fonte: {fonte.name}</p>}
                    {t.attachment && !isImage && <p style={{ fontSize: 12, color: COLORS.muted, margin: "4px 0 0" }}><Paperclip size={11} /> {t.attachment}</p>}
                  </div>
                  <p style={{ fontSize: 15, fontWeight: 600, margin: 0, color: t.type === "income" ? COLORS.income : COLORS.rust }}>{t.type === "income" ? "+" : "−"}{fmt(t.amount)}</p>
                </div>
          ...
          <p style={{ fontSize: 13, color: COLORS.muted, margin: "6px 0 0" }}>Total pago: <strong>{fmt(paidTotal)}</strong> de {fmt(item.amount)}</p>
        </div>
      ) : (
        <>
          ...
          <FormField label="Anexar comprovante (opcional)">
            <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
              {ATTACH_METHODS.map(([v, l]) => (
                <button key={v} onClick={() => setAttachmentMethod(v)} style={{ flex: 1, padding: "7px 0", borderRadius: 8, fontSize: 12, fontWeight: 500, border: "1px solid " + (attachmentMethod === v ? COLORS.green : COLORS.line), background: attachmentMethod === v ? COLORS.green : "transparent", color: attachmentMethod === v ? "#fff" : COLORS.ink }}>{l}</button>
              ))}
            </div>
            {(attachmentMethod === "anexo" || attachmentMethod === "foto") ? (
              ...
              <>
                <button onClick={() => setShowScanner(true)} style={{ width: "100%", padding: "9px 0", borderRadius: 10, border: "1px solid " + COLORS.green, background: COLORS.green + "12", color: COLORS.green, fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Escanear pela câmera</button>
          ...
          {onFinalize && (
            <button onClick={() => onFinalize(item)} style={{ width: "100%", padding: "10px 0", borderRadius: 10, border: "1px solid " + COLORS.green, background: COLORS.green + "12", color: COLORS.green, fontSize: 14.5, fontWeight: 500, marginTop: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
