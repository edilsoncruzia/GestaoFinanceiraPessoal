    ...
    <ModalSheet title="Holerite — registrar recebimento" onClose={onClose}>
      <Card style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <p className="serif" style={{ fontSize: 21, fontWeight: 600, margin: 0, color: COLORS.ink }}>{fmt(gross)}</p>
        </div>
        <p style={{ fontSize: 13, color: COLORS.muted, margin: 0 }}>Salário bruto</p>
      </Card>
      ...
      <p style={{ fontSize: 13, fontWeight: 600, color: COLORS.muted, margin: "12px 0 8px", textTransform: "uppercase", letterSpacing: 0.4 }}>Descontos em folha</p>
      {deductions.length === 0 && <p style={{ fontSize: 14, color: COLORS.muted, margin: "0 0 10px" }}>Nenhum desconto configurado.</p>}
      {deductions.map((d, idx) => (
        <div key={idx} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 14.5, fontWeight: 500, margin: 0, color: COLORS.ink }}>{d.label}</p>
          </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", margin: "12px 0 4px", paddingTop: 10, borderTop: "1px solid " + COLORS.line }}>
        <span style={{ fontSize: 14, color: COLORS.muted }}>Total de descontos</span>
        <span style={{ fontSize: 14, fontWeight: 600, color: COLORS.rust }}>− {fmt(totalDeductions)}</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: COLORS.ink }}>Salário líquido</span>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, margin: "4px 0 14px", cursor: "pointer" }}>
        ...
        <span style={{ fontSize: 14, color: COLORS.ink }}>Salvar estes valores como modelo para os próximos meses</span>
      </label>
      ...
      <p style={{ fontSize: 12, color: COLORS.muted, margin: "10px 0 0", textAlign: "center" }}>Registra a receita bruta e as despesas dos descontos de uma só vez. O líquido é o que entra como disponível.</p>
    </ModalSheet>
  );
}
